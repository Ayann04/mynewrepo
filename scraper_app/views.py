...
def trigger_scrape(request):
    ...
    try:
        _create_status(new_run, "Your Process Has Been Started")
        driver = _driver_from_config()
        _create_status(new_run, "Website Is Loading.....")
        driver.get("https://sampada.mpigr.gov.in/#/clogin")
        time.sleep(20)
        english_to = driver.find_elements(By.CSS_SELECTOR,'div.ng-star-inserted>a')
        english_to[2].click()
        _create_status(new_run, "Clicked On English Language")
        ...
        try:
            ...
            time.sleep(10)
            _create_status(new_run, "Captcha Is Loading Please Wait.")
            ...
            print("Exception occurred:")
            traceback.print_exc()
            _create_status(new_run, f"something wrong {e}")
            
        _create_status(new_run, "Captcha-1 Solved Sucessfully") 
        # Navigate to search
        ...
        _create_status(new_run, "Logged In Waiting To Load Sites.")
        ...
        try:
            ...
            _create_status(new_run, "Filling Necessary Details For Fetching The Data.")
            time.sleep(20)
            print(date_from_fmt)
            ...
            _create_status(new_run, "Filling The Captcha",)
            time.sleep(2)
        ...
        try:
            while True:  # Keep looping through all pages until no next button
                    ...
                    _create_status(new_run,f"Scrapping Started ",) 
                    for i in range(len(data_elements_2)):
                        if i >= len(data_elements_2):
                            break
                        span = data_elements_2[i]
                        _create_status(new_run,f"Scraping Started On Data{i}",)  
                        ...
                        try:
                            ...
                            _create_status(new_run,f"Some Error {e}",) 
        ...
        print("Exception occurred:")
        traceback.print_exc()
        _create_status(new_run, "Scraping failed due to an error.{e}")
    ...
    try:
        ...
        if run_id:
            try:
                ...
                logging.warning(f"ScrapingRun with id={run_id} not found. Falling back to latest run.")

        if run is None:
            try:
                run = ScrapingRun.objects.order_by("-id").first()
            except Exception:
                run = None

        # Build a deterministic worksheet title per run
        if run is not None:
            started_at = getattr(run, "started_at", None) or getattr(run, "created_at", None) or datetime.now()
            timestamp_part = started_at.strftime("%Y-%m-%d_%H-%M-%S")
            sheet_title = f"Run_{run.id}_{timestamp_part}"
        else:
            timestamp_part = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            sheet_title = f"Run_{timestamp_part}"
        ...
        spreadsheet = client.open("sampada-scraping")

        # Get or create the worksheet for this run
        try:
            sheet = spreadsheet.worksheet(sheet_title)
            worksheet_exists = True
        except gspread.exceptions.WorksheetNotFound:
            worksheet_exists = False
            sheet = spreadsheet.add_worksheet(title=sheet_title, rows=5000, cols=50)

        # Select records for this run
        # If ScrapedRecord has a 'run' relation, filter by it; otherwise export all.
        try:
            has_run_fk = any(f.name == "run" for f in ScrapedRecord._meta.get_fields())
        except Exception:
            has_run_fk = False

        if has_run_fk and run is not None:
            records = ScrapedRecord.objects.filter(run=run).order_by("id")
        else:
            records = ScrapedRecord.objects.all().order_by("id")

        # If worksheet already populated (>1 row), assume already exported; redirect
        existing_values = sheet.get_all_values()
        if worksheet_exists and len(existing_values) > 1:
            google_sheet_link = f"https://docs.google.com/spreadsheets/d/{spreadsheet.id}/edit#gid={sheet.id}"
            return redirect(google_sheet_link)

        # No data case
        if not records.exists():
            if not existing_values:
                sheet.update("A1", [["No data"]])
            google_sheet_link = f"https://docs.google.com/spreadsheets/d/{spreadsheet.id}/edit#gid={sheet.id}"
            return redirect(google_sheet_link)

        # Build consistent headers across all records with prefixed sections
        # We collect ordered unique keys for each section.
        def section_columns(section_attr: str, prefix: str):
            ordered = OrderedDict()  # label -> original_key
            for r in records:
                data = getattr(r, section_attr, None) or {}
                for k in list(data.keys()):
                    label = f"{prefix}{k}"
                    if label not in ordered:
                        ordered[label] = k
            return list(ordered.items())  # [(label, original_key), ...]

        reg_cols = section_columns("registration_details", "Registration - ")
        sel_cols = section_columns("seller_details", "Seller - ")
        buy_cols = section_columns("buyer_details", "Buyer - ")
        prop_cols = section_columns("property_details", "Property - ")
        kha_cols = section_columns("khasra_details", "Khasra - ")

        headers = [lbl for (lbl, _) in (reg_cols + sel_cols + buy_cols + prop_cols + kha_cols)]

        # Column spec tells us from which section and key to pull a value for each header cell
        col_spec = (
            [("registration_details", k) for (_, k) in reg_cols] +
            [("seller_details", k) for (_, k) in sel_cols] +
            [("buyer_details", k) for (_, k) in buy_cols] +
            [("property_details", k) for (_, k) in prop_cols] +
            [("khasra_details", k) for (_, k) in kha_cols]
        )

        # Build rows following the header order
        rows = []
        for r in records:
            row = []
            for section_attr, key in col_spec:
                value = (getattr(r, section_attr, None) or {}).get(key, "")
                # Normalize to string to avoid issues with non-primitive JSON values
                row.append("" if value is None else str(value))
            rows.append(row)

        # Write header + data; clear partial content if any
        if existing_values:
            sheet.clear()
        ...
        # Redirect to the concrete worksheet
        google_sheet_link = f"https://docs.google.com/spreadsheets/d/{spreadsheet.id}/edit#gid={sheet.id}"
        return redirect(google_sheet_link)

    except Exception as e:
        logging.exception("Error exporting to Google Sheets")
